...
- Git vs. GitHub, and the basic push/pull loop
...
A version control system (*DC* §9.1) lets you:
...
Even if you're working solo (*DC* §9.1.1), version control pays off:
...
::: {.callout-note}
Paired with **literate programming** (your Quarto documents from week 1), version control is what makes an analysis *reproducible*: a complete record from raw data to final result (*DC* §9.1.2).
:::
...
Most days, the whole workflow fits in four steps (*DC* §9.2.2):
...
Think of the remote as the shared copy everyone syncs to (*DC* §9.2). For your project team:
...
Now it's your turn. The activity walks you through each step.

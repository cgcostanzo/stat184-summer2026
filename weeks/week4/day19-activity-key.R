...
# Casey doesn't have a "year" tag. Instead of gravving individual years, we
# should create a container with each student in it. We convert thisto a data 
# frame, which is much more useful.
...
# html_element() searches inside each student's <li> and returns one result for
# each student, using NA when no match exists. html_elements() doesn't use NA,
# and instead just omits cases where no match exists.  

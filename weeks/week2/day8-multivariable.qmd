...
The grammar of graphics is the *vocabulary*. The DC "specify a graph" steps are the *grammar*, or the order you put the words in.
:::
:::
...
## Two variables {.scrollable .nostretch}
...
## Three variables, with color {.scrollable .nostretch}
...
The negative trend we saw before was **Simpson's paradox**, i.e., *within* each species, longer bills are deeper. The pooled trend ran the wrong way because Gentoos (long bills, shallow bills) are a separate cluster from the other species.

## Three variables, with shape {.scrollable .nostretch}
...
## Four variables: combine color and shape {.scrollable .nostretch}
...
## Five variables: add a facet {.scrollable .nostretch}

Instead of mapping a 5th aesthetic, try faceting instead:
...
## `facet_wrap()` vs `facet_grid()` {.scrollable .nostretch}
...
## The `.` placeholder in `facet_grid()` {.scrollable .nostretch}
...
## When rows beat columns (or vice versa) {.scrollable .nostretch}
...
## Free vs. fixed scales in facets {.scrollable .nostretch}
...
## When to facet vs. when to use color {.scrollable .nostretch}
...
## The DC workflow {.scrollable .nostretch}
...
## Worked example: bill length vs. depth {.scrollable .nostretch}
...
## Putting it all together {.scrollable .nostretch}
...
## Labels are not optional {.scrollable .nostretch}
...
## DC's axis-customization toolkit {.scrollable .nostretch}
...
## When to log-transform an axis {.scrollable .nostretch}
...
## Log axis fixes it {.scrollable .nostretch}
...
## Continuous color scales {.scrollable .nostretch}
...
## Sequential vs. diverging palettes {.scrollable .nostretch}
...
## When you might change the coord {.scrollable .nostretch}
...
## Long labels: swap x and y {.scrollable .nostretch}
...
## Where we are {.scrollable .nostretch}
...
- **Coordinate systems:** flip, fixed, polar. This is useful for a few specific cases
